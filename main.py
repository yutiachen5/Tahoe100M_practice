import numpy as np
from datasets import TahoeDataset
from models import Transformer
from pad import pad_genes
...
def train(model, train_loader, val_loader, num_epoches, use_cuda, optimizer):
    if use_cuda:
        model.cuda()
    ...
    for i in range(1, num_epoches+1):
        ...
        for genes, expressions, smiles, targets, padding_masks in enumerate(train_loader):
            if use_cuda:
...
def main():
    ...
    parser.add_argument('--gene-emb-dim', type=int, defaule=128, help='embeding dimension of gene and gene expression (default:128)')

    parser.add_argument('--data', default='/hpc/home/yc583/Tahoe100M_practice/data/Tahoe_1M_merged.csv', help='path to training dataset')
    parser.add_argument('--pretrained-mdl', type=str, default='/hpc/home/yc583/Tahoe100M_practice/pretrained_molformer', help='base path to pretrained smiles transformer')
    ...
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, collate_fn=pad_genes)
    val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=True, collate_fn=pad_genes)
    ...
    model = Transformer(gene_emb_dim=args.gene_emb_dim, num_genes=num_genes, emb_dim=args.d_model, num_heads=args.nhead, num_layers=args.nlayer, 
                        pretrained_smiles_mdl = args.pretrained_mdl,dim_feedforward=args.dim_feedforward, 
                        dropout=args.dropout, max_len=args.max_length)
    ...
    train(model,  train_loader, val_loader, args.n, use_cuda, optimizer)
